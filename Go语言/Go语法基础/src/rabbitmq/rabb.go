...
func (r *RabbitMQ)mqConnect() {
	...
	mqConn, err = amqp.Dial("amqp://guest:guest@10.253.100.29:5672//")
...
//func (c *QueueExchange) IsClosed() bool {
...
func (r *RabbitMQ) listenReceiver(receiver Receiver) {
	...
	// 绑定任务
	err =  r.channel.QueueBind(r.queueName, r.routingKey, r.exchangeName, true, r.ares)
	if err != nil {
		fmt.Printf("绑定队列失败:%s \n", err)
		return
	}
	...
	msgList, err :=  r.channel.Consume(
		r.queueName, "", false, false, false, false, r.ares)
	if err != nil {
...
func main() {
	...
	//var tapye map[string]interface{}
	tapye := make(map[string]interface{})
	tapye["x-max-priority"]=3
